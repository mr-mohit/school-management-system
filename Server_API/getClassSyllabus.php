	...
	if($_SERVER['REQUEST_METHOD'] == 'POST')
	{
        ...
        $obj=json_decode($postdata,true);
			...
			else
			{
				...
				$sqls = "SELECT DISTINCT SUBJECT_ID FROM `class_reg` WHERE `CLASS_MASTER_ID`=  '$classID' ";
				$query=mysqli_query($con,$sqls);
				if ($count == 0)
				{					 
				   $response['message'] = "No Subjects, No Syllabus";
				   $response['statuscode']=0;
				   echo json_encode($response); 
				}
				else
				...
				// select Syllabus
				$sql = "SELECT * FROM `class_reg` WHERE `CLASS_MASTER_ID` = '$classID'";
				$query=mysqli_query($con,$sql);	
				if ($count == 0)
				{					 
				   $response['message'] = "No Syllabus Available for this class";
				   $response['statuscode']=0;
				   echo json_encode($response); 
				}
				else
